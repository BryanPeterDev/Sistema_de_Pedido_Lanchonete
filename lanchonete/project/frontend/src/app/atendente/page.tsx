...
function AtendentePage() {
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      {/* ── Right: Order Summary ────────────────────────────────── */}
      <div className="w-[380px] bg-white border-l border-surface-100 flex flex-col overflow-hidden shadow-2xl">
                    ...
                    <div key={item.cartId} className="bg-surface-50 rounded-xl p-3 space-y-2">
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                          <button onClick={() => updateQty(item.cartId, -1)} className="w-7 h-7 rounded-lg bg-white border border-surface-200 flex items-center justify-center text-surface-800 hover:border-brand-400 transition-colors">
                            <Minus size={12} />
                          </button>
                          ...
                          <button onClick={() => removeFromCart(item.cartId)} className="w-7 h-7 rounded-lg flex items-center justify-center text-red-400 hover:bg-red-50 transition-colors ml-1">
                            <Trash2 size={12} />
                          </button>
                        </div>
                      </div>
      ...
      {selectedProductForOptions && (
        <Modal open={!!selectedProductForOptions} onClose={() => setSelectedProductForOptions(null)} title={`Opções: ${selectedProductForOptions.name}`}>
              ...
              return (
                <div key={group.id} className="space-y-3">
                  <div className="flex justify-between items-end">
                    ...
                    <span className="bg-surface-100 text-surface-500 text-[11px] font-body px-2 py-0.5 rounded-md">
                  ...
                  <div className="grid gap-2">
                    {group.options.map((opt) => {
                      ...
                      return (
                        <div key={opt.id} className={cn("flex justify-between items-center p-3 rounded-xl border transition-all", isSelected ? "border-brand-500 bg-brand-50 shadow-sm" : "border-surface-200 bg-white hover:border-brand-300")}>
                          ...
                          <div className="flex items-center gap-3">
                            {group.option_type === "single" ? (
                              <button onClick={() => setOptionSelections(prev => ({ ...prev, [group.id]: { [opt.id]: 1 } }))} className={cn("w-6 h-6 rounded-full border-2", isSelected ? "bg-brand-500 border-brand-500" : "bg-white border-surface-200")} />
                            ) : (
                              <div className="flex items-center gap-2">
                                {isSelected && (
                                  <>
                                    <button onClick={() => setOptionSelections(prev => {
                                      const curr = { ...(prev[group.id] || {}) };
                                      if (curr[opt.id] > 1) curr[opt.id] -= 1; else delete curr[opt.id];
                                      return { ...prev, [group.id]: curr };
                                    })} className="w-7 h-7 rounded-lg border border-surface-200 flex items-center justify-center"><Minus size={14} /></button>
                                    <span className="w-4 text-center text-sm font-body font-bold">{qty}</span>
                                  </>
                                )}
                                <button onClick={() => setOptionSelections(prev => {
                                  const curr = { ...(prev[group.id] || {}) };
                                  if (totalInGroup < effectiveMax) curr[opt.id] = (curr[opt.id] || 0) + 1;
                                  return { ...prev, [group.id]: curr };
                                })} className="w-7 h-7 rounded-lg bg-brand-500 text-white flex items-center justify-center"><Plus size={14} /></button>
                              </div>
                            )}
                          </div>
                        </div>
          ...
          <div className="mt-8 pt-4 border-t border-surface-100 flex justify-end gap-3">
            <Button variant="secondary" onClick={() => setSelectedProductForOptions(null)}>Cancelar</Button>
            <Button onClick={confirmAddToCartWithOptions}>Confirmar</Button>
          </div>
        </Modal>
      )}
    </div>
