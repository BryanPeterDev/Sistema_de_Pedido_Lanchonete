...
import { ShoppingBag, Package, Tag, Boxes, Users, PlusCircle, ChefHat, Truck, LogOut, LayoutGrid, BarChart2, DollarSign, FileText, PercentCircle, MoreVertical } from "lucide-react";
...
const sections = [
  ...
  },
  {
    ...
    links: [
      ...
      { href: "/admin/promocoes", label: "Promoções", icon: PercentCircle },
...
export default function AdminSidebar() {
  ...
  return (
    <aside className={cn(
        ...
        <div className={cn("flex flex-col items-center transition-all duration-500", isCollapsed ? "mb-6" : "mb-6")}>
          ...
          {!isCollapsed && (
            <div className="animate-fade-in text-center">
              ...
              <p className="text-[10px] text-brand-500 font-body font-bold uppercase tracking-[0.2em] mt-0.5">Food Truck</p>
            </div>
          )}
        </div>
      ...
      {/* Nav */}
      <nav className="flex-1 p-4 space-y-5 overflow-y-auto overflow-x-hidden scrollbar-hide">
        {sections.map((section) => (
          <div key={section.title}>
            {!isCollapsed && (
              <p className="text-[10px] font-body font-bold text-white/40 tracking-[0.2em] mb-3 px-4 animate-fade-in uppercase">
                {section.title}
              </p>
            )}
            <div className="space-y-1">
              {section.links.map(({ href, label, icon: Icon }) => (
                ...
                >
                  <Icon size={20} className={cn("transition-transform duration-300 group-hover:scale-110", pathname.startsWith(href) ? "text-white" : "text-inherit")} />
        ...
        <div className={cn("flex items-center gap-3 p-2 rounded-2xl bg-surface-900/40 border border-surface-800/30 mb-2", isCollapsed ? "justify-center" : "")}>
          ...
          {!isCollapsed && (
            <div className="flex-1 min-w-0 animate-fade-in">
              ...
              <p className="text-[9px] text-surface-500 font-body uppercase tracking-tighter">Admin</p>
            </div>
          )}
        </div>
        <button
