...
T_CurrenteUser = Annotated[User, Depends(get_current_user)]
...
def create_user(user: UserSchema, session: T_Session):
    ...
    if db_user:
        if db_user.username == user.username:
            raise HTTPException(
                status_code=HTTPStatus.BAD_REQUEST, detail='Username ja existe',)
...
def update_user(user_id: int, user: UserSchema, session: T_Session, current_user: T_CurrenteUser):
...
def delete_user(user_id: int, session: T_Session, current_user: T_CurrenteUser):
